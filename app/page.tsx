...
export default async function AdminDashboard({ searchParams }: PageProps) {
  ...
  const contentSource = source === 'user' ? 'user' : 'system';
  const categories = await getCategoriesWithQuestionCount(q, contentSource as 'system' | 'user');

  const totalQuestions = categories.reduce((sum: number, cat: CategoryWithCounts) => sum + cat._count.questions, 0);
  const isReadOnly = contentSource === 'user';
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 font-sans selection:bg-indigo-500/30">
      ...
      <main className="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <ActionBar initialQuery={q} source={contentSource} readOnly={isReadOnly} />

        {/* Categories Grid */}
        {categories.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categories.map((category: CategoryWithCounts) => (
              <CategoryCard key={category.id} category={category} readOnly={isReadOnly} />
            ))}
          </div>
        ) : (
